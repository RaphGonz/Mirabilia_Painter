# Stack Research — Paint AI

**Project:** Paint AI (DDPG + differentiable neural renderer, Learning to Paint reimplementation)
**Researched:** 2026-06-08
**Mode:** Ecosystem — standard 2025 stack for RL-based painting systems
...
**Install command (Windows, CUDA 12.6):**
```bash
...
**Verdict: Implement DDPG from scratch directly in PyTorch. Do NOT use Stable-Baselines3 or TorchRL for this project.**

Rationale is detailed in the "What NOT to Use" section. Here is the self-contained pattern to follow:
...
**Key DDPG hyperparameters (starting point):**

```python
GAMMA = 0.99        # discount
TAU = 0.005         # soft update rate (not 0.001 — too slow for painting)
BATCH_SIZE = 256
BUFFER_SIZE = 500_000
ACTOR_LR = 1e-4
CRITIC_LR = 1e-3
...
WARMUP_STEPS = 5_000  # random actions before training starts
```
...
The renderer R is a supervised CNN/MLP trained to map stroke params → 64×64 stroke image. It is trained once, then frozen (no grad) during DDPG training.
...
**Reference architecture (starting point):**
```python
class NeuralRenderer(nn.Module):
    def __init__(self, stroke_dim=8, img_size=64):
        super().__init__()
        self.fc = nn.Sequential(
            nn.Linear(stroke_dim, 512), nn.ReLU(),
            nn.Linear(512, 1024), nn.ReLU(),
            nn.Linear(1024, 4 * 4 * 256), nn.ReLU(),
        )
    ...
    def forward(self, params):
        x = self.fc(params).view(-1, 256, 4, 4)
...
**TensorBoard usage pattern:**
```python
from torch.utils.tensorboard import SummaryWriter
writer = SummaryWriter("runs/paint_ddpg_run1")
writer.add_scalar("reward/episode_mean", mean_reward, global_step)
writer.add_scalar("loss/critic", critic_loss.item(), global_step)
writer.add_scalar("loss/actor", actor_loss.item(), global_step)
writer.add_image("canvas/vs_target", comparison_grid, global_step)  # every N steps
```

**Timelapse export pattern:**
```python
import imageio
frames = []  # collect (H,W,C) uint8 numpy arrays during eval
with imageio.get_writer("timelapse.gif", mode="I", duration=0.1) as writer:
    for frame in frames:
...
**Recommended project virtual env setup:**
```bash
conda create -n paint_ai python=3.11
conda activate paint_ai
pip install torch torchvision --index-url https://download.pytorch.org/whl/cu126
pip install tensorboard matplotlib imageio imageio-ffmpeg opencv-python numpy tqdm
pip install jupyterlab  # for prototyping only
```
...
SB3 wraps the environment in its own training loop and data collection pipeline. The Paint AI environment is non-standard: the "reward" flows through a frozen neural renderer R, and the observation is a custom 6-channel image tensor. Wiring SB3's `CnnPolicy` around a custom renderer requires fighting the library's assumptions about feature extractor architecture. The policy network needs to share the CNN backbone between actor and critic in a specific way that matches the painting task — SB3 makes this awkward. Building DDPG from scratch in ~200 lines of PyTorch is faster, fully transparent, and gives direct control over the gradient flow through R.

### TorchRL

**Do not use** for this project (at this stage).

TorchRL 0.13 is powerful but introduces significant cognitive overhead: TensorDict data containers, env wrappers, collectors, and a module system that requires understanding before it helps. For a greenfield reimplementation of a specific paper, the overhead of learning TorchRL's abstractions exceeds the benefit. TorchRL's real value is in distributed collection, vectorized envs, and complex data pipelines — none of which apply at 64×64 with a single GPU. If the project scales to multi-env parallel collection in a future episode, revisit TorchRL then.

### RLlib (Ray)

**Do not use.** Distributed-first framework with heavy config overhead. Completely overkill for a single-GPU local experiment. Adds Ray as a dependency with significant install weight on Windows.

### OpenAI Gym (legacy, pre-1.0)

**Do not use `gym` (the old package).** Use `gymnasium` (the maintained Farama fork) if you need a formal env interface. For this project, since the environment is entirely custom and not shared with other frameworks, you may skip `gymnasium` entirely and implement `reset()` / `step()` as plain Python methods on a class. This avoids the overhead of matching the Gymnasium API contract for no benefit.

### TensorFlow / JAX

**Do not use.** The project is locked to PyTorch by design. No reason to introduce a second framework.

### Weights & Biases (wandb)

**Do not use** for the baseline. wandb requires an account, uploads data to the cloud, and adds latency to training steps via its callback hooks. TensorBoard runs entirely locally with zero config. Use wandb only if collaboration or remote experiment access becomes a requirement in a future episode.

### MLP-only Neural Renderer

**Do not use** a pure MLP for R (e.g., `Linear(8) → ... → Linear(4096)`). At 64×64=4096 output pixels, an MLP has no spatial structure and requires a very wide network to generalize across stroke geometries. A conv decoder with upsampling is the correct inductive bias for this task and trains faster with better visual quality.

### PIL/Pillow as primary image handler

**Do not use** Pillow as the main in-training image handler. PIL Images require conversion to tensors via `torchvision.transforms.ToTensor()` on every operation, which is slow in a tight training loop. Keep everything as PyTorch tensors on GPU throughout the training loop; convert to numpy/PIL only for logging and visualization.

### autograd through the hard rasterizer

**Do not attempt** to make the hard rasterizer differentiable (e.g., via soft rasterization tricks or `torch.autograd.Function`). The design explicitly separates R (differentiable, frozen) from the hard rasterizer (ground truth, no grad). Adding autograd to the hard rasterizer contradicts the architecture and adds complexity for no benefit in episode 1.
